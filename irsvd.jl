#943 users
#1682 movies
#baseado na implementação  https://github.com/alabid/PySVD
using Distributions, DataFrames, PyPlot
function irsvdPredict(userId,itemId,U,V,c,d)
  u = U[userId,:]
  v = V[itemId,:]
  r =(c[userId] + d[itemId]) + vecdot(u',v)
  if r > 5.0
    return 5.0
  elseif r < 1.0
    return 1.0
  end
  return r

end
#train a k column
function irsvdTrainK(usersItemsRating,U,V,k,c,d)
lrate = 0.001
λ = 0.02
λ2 = 0.05
global_mean = 3.6033
rmse = 0.0

for index = 1:size(usersItemsRating,1)
    userId = usersItemsRating[index,1]
    itemId = usersItemsRating[index,2]
    trueRating = usersItemsRating[index,3]
    err = trueRating - irsvdPredict(userId,itemId,U,V,c,d)
    rmse += err^2
    u = U[userId,k]
    v = V[itemId,k]
    C = c[userId]
    D = d[itemId]
    U[userId,k] += lrate*(err*v - λ*u)
    V[itemId,k] += lrate*(err*u - λ*v)
    c[userId] += lrate*(err-λ2*(C + D - global_mean))
    d[itemId] += lrate*(err-λ2*(C + D - global_mean))
end

return U,V,sqrt(rmse/size(usersItemsRating,1))
end

function irsvdTrainBase(usersItemsRating,k_)
  U = zeros(Float64,totalUsers,k)
  V = zeros(Float64,totalItems,k)
  c = zeros(Float64,totalUsers)
  d = zeros(Float64,totalItems)
  #inicializa U e V com valores aleatorios seguindo uma distribuicao normal
  for i = 1:totalUsers,j = 1:k
  U[i,j] = 0.1
  c[i] = 0.01
  end
  for i = 1:totalItems,j = 1:k
  V[i,j] = 0.1
  d[j] = 0.01
  end
maxIter = 1000
minErr = 0.0001
previousErr = 1000000.0
for aux in 1:k_
  println(aux)
  for count in 1:maxIter
    U,V,rmse = irsvdTrainK(usersItemsRating,U,V,aux,c,d)
    if abs(previousErr - rmse) < minErr
       break
    end
    previousErr = rmse
  end
end
return U,V,c,d
end

function rsvdRmse(U,V,c,d,usersItemsRating)
rmse = 0.0
predicts = zeros(size(usersItemsRating,1))
for index in 1:size(usersItemsRating,1)
  userId = usersItemsRating[index,1]
  itemId = usersItemsRating[index,2]
  trueRating = usersItemsRating[index,3]
  predicts[index]= irsvdPredict(userId,itemId,U,V,c,d)
  err = trueRating - irsvdPredict(userId,itemId,U,V,c,d)
  rmse += err^2
end
return sqrt(rmse/size(usersItemsRating,1)),predicts
end

#Global variables
u_col_names=[:user_id, :item_id, :rating, :timestamp]
k = 96
totalUsers = 943
totalItems = 1682
treinamento1 = DataFrames.readtable("ml-100k/u1.base", separator=' ', header=false, names=u_col_names)
teste1 = users = DataFrames.readtable("ml-100k/u1.test", separator=' ', header=false, names=u_col_names)

u,v,C,D = irsvdTrainBase(treinamento1,k)
rmse, predicts = rsvdRmse(u,v,C,D,teste1)#printa o rmse utilizando o teste como base
println("rmse para o teste1 $rmse")
real = teste1[:,3]
#sort!(predicts)
#sort!(real)
#plot(predicts,1:size(teste1,1),color = "red",linewidth=2.0,linestyle="-")
#plot(real,1:size(teste1,1),color = "blue",linewidth=2.0,linestyle="-")
